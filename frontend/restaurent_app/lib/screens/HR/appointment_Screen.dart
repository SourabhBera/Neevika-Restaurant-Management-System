...
class AppointmentLetterScreen extends StatefulWidget {
  const AppointmentLetterScreen({super.key});

  @override
  State<AppointmentLetterScreen> createState() => _AppointmentLetterScreenState();
}

class _AppointmentLetterScreenState extends State<AppointmentLetterScreen> {
  ...
  Future<void> _uploadAppointmentLetter(int userId) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      final uri = Uri.parse('${dotenv.env['API_URL']}/hr/appointment/$userId');
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Appointment letter uploaded successfully')),
          );
          await _fetchUsers();
        } else {
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: DataTable(
                              ...
                              rows: _filtered.map((user) {
                                final hasLetter = user.appointmentLetterPath != null && user.appointmentLetterPath!.isNotEmpty;
                                ...
                                return DataRow(
                                  cells: [
                                    ...
                                    DataCell(Row(
                                      children: [
                                        TextButton.icon(
                                          onPressed: () => _uploadAppointmentLetter(user.id),
...
class UserRecord {
  ...
  factory UserRecord.fromJson(Map<String, dynamic> json) {
    return UserRecord(
      ...
      appointmentLetterPath: json['appointment_letter_path'],
    );
  }
}
