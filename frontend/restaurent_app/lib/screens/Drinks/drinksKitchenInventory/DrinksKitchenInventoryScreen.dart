import 'package:Neevika/screens/Drinks/drinksKitchenInventory/DrinksBarcodeTransferScreen.dart';
...
import 'package:Neevika/screens/Drinks/drinksKitchenInventory/DrinksTransferHistory.dart';
...
class DrinksKitchenInventoryPage extends StatefulWidget {
  const DrinksKitchenInventoryPage({super.key});

  @override
  _DrinksKitchenInventoryPageState createState() => _DrinksKitchenInventoryPageState();
}

class _DrinksKitchenInventoryPageState extends State<DrinksKitchenInventoryPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          print('\n\n MAIN inventory $mainInventoryItems\n\n\n');
  ...
  Future<void> fetchkitchenInventory() async {
    try {
      final response = await http
          .get(
            Uri.parse(
              '${dotenv.env['API_URL']}/drinks-kitchen-inventory/inventory/',
            ),
          )
          .timeout(const Duration(seconds: 6));
  ...
  final String apiUrl = '${dotenv.env['API_URL']}/drinks-kitchen-inventory/transfer/';
  ...
  return ElevatedButton.icon(
    onPressed: () async {
      await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => DrinksBarcodeTransferScreen()),
      );
...
Widget buildAddButton() {
  return ElevatedButton.icon(
    onPressed: () {
      Navigator.push(context, MaterialPageRoute(builder: (context) => DrinksTransferHistoryPage()),);
    },
            style: ElevatedButton.styleFrom(
              backgroundColor: Color(0xFFD95326),
              elevation: 0,
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: GoogleFonts.poppins(fontSize: 10.2, color: Colors.grey),
            ),
            SizedBox(height:10),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              TransferItemsCard(
                ...
                onTransferPressed: () {
                  // Define what happens when the transfer button is pressed
...
final filteredSource = source.where((item) {
  final itemName = widget.selectedTab == "Kitchen"
      ? item["drinksInventory"]["itemName"]?.toString().toLowerCase() ?? ''
  ...
  return itemName.contains(lowerQuery);
    ...
    return filteredSource.map<Map<String, String>>((item) {
      if (widget.selectedTab == "Kitchen") {
        return {
          "item": item["drinksInventory"]["itemName"]?.toString() ?? '',
  ...
  void _handleTabChange(String tab) {
    ...
    final url = Uri.parse(
      '${dotenv.env['API_URL']}/drinks-kitchen-inventory/download',
    );
  ...
  Future<void> downloadInventoryFile(BuildContext context) async {
    final url = Uri.parse(
      '${dotenv.env['API_URL']}/drinks-inventory/download',
    );
  ...
  TableRow _buildTableRow(Map<String, String> item) {
    ...
    return TableRow(
      children: [
        ...
        //   child: Text('item["peg"]!', style: GoogleFonts.poppins(fontSize: 11),),
        // ),
        Padding(
